...
export class Table {
  ...
  constructor(name: string, database: Database, firstDefinitionPage: number) {
    ...
    this.#rowCount = this.#definitionBuffer.readUInt32LE(
      this.#database.format.tableDefinitionPage.rowCountOffset,
    );

    this.#columnCount = this.#definitionBuffer.readUInt16LE(
      this.#database.format.tableDefinitionPage.columnCountOffset,
    );
    ...
    const usageMapBuffer = this.#database.findPageRow(
      this.#definitionBuffer.readUInt32LE(
        this.#database.format.tableDefinitionPage.usageMapOffset,
      ),
    );
  ...
  #getColumnDefinitions(): ColumnDefinition[] {
    ...
    let curDefinitionPos =
      ...
      this.#realIndexCount *
        this.#database.format.tableDefinitionPage.realIndexEntrySize;

    let namesCursorPos =
      curDefinitionPos +
      this.#columnCount *
        this.#database.format.tableDefinitionPage.columnsDefinition.entrySize;
    ...
    for (let i = 0; i < this.#columnCount; ++i) {
      ...
      const type = getColumnType(
        this.#definitionBuffer.readUInt8(
          curDefinitionPos +
            this.#database.format.tableDefinitionPage.columnsDefinition
              .typeOffset,
        ),
      );
  ...
  >(
    ...
    const columns = columnDefinitions.filter(
      (c) => options.columns === undefined || options.columns.includes(c.name),
    );
    ...
    for (const dataPage of this.#dataPages) {
      ...
      const recordOffsetsToLoad = recordOffsets.slice(
        rowsToSkip,
        rowsToSkip + rowsToRead,
      );
      const recordsOnPage = this.#getDataFromPage(
        pageBuffer,
        recordOffsetsToLoad,
        columns,
      );
  ...
  #getRecordOffsets(pageBuffer: Buffer): RecordOffset[] {
    const recordCount = pageBuffer.readUInt16LE(
      this.#database.format.dataPage.recordCountOffset,
    );
    const recordOffsets: RecordOffset[] = [];
    for (let record = 0; record < recordCount; ++record) {
      const offsetMask = 0x1fff;

      let recordStart = pageBuffer.readUInt16LE(
        this.#database.format.dataPage.record.countOffset + 2 + record * 2,
      );
      ...
      const nextStart =
        record === 0
          ...
          : pageBuffer.readUInt16LE(
              this.#database.format.dataPage.record.countOffset + record * 2,
            ) & offsetMask;
  ...
  #getDataFromPage(
    ...
    for (const [recordStart, recordEnd] of recordOffsets) {
      const rowColumnCount = pageBuffer.readUIntLE(
        recordStart,
        this.#database.format.dataPage.record.columnCountSize,
      );
      ...
      if (this.#variableColumnCount > 0) {
        switch (this.#database.format.dataPage.record.variableColumnCountSize) {
          case 1: {
            rowVariableColumnCount = pageBuffer.readUInt8(
            ...
            if (
              (columnPointer - recordStart - rowVariableColumnCount) / 256 <
              jumpCount
            ) {
              --jumpCount;
            }
            ...
            for (let i = 0; i < rowVariableColumnCount + 1; ++i) {
              while (
                jumpsUsed < jumpCount &&
                i ===
                  pageBuffer.readUInt8(recordEnd - bitmaskSize - jumpsUsed - 1)
              ) {
                ++jumpsUsed;
              }
              variableColumnOffsets.push(
                pageBuffer.readUInt8(columnPointer - i) + jumpsUsed * 256,
              );
            }
            break;
          }
          case 2: {
            rowVariableColumnCount = pageBuffer.readUInt16LE(
            ...
            for (let i = 0; i < rowVariableColumnCount + 1; ++i) {
              variableColumnOffsets.push(
                pageBuffer.readUInt16LE(recordEnd - bitmaskSize - 3 - i * 2),
              );
            }
            break;
          }
      ...
      for (const column of [...columns].sort((a, b) => a.index - b.index)) {
        ...
        if (column.fixedLength && fixedColumnsFound < rowFixedColumnCount) {
          ...
          !column.fixedLength &&
          column.variableIndex < rowVariableColumnCount
        ) {
