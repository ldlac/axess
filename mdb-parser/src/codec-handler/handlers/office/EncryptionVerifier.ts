...
export function parseEncryptionVerifier(
  ...
  const encryptionVerifier = encryptionProvider.slice(
    encryptionVerifierOffset,
    verifierHashSizeOffset,
  );
  const encryptionVerifierHashSize = encryptionProvider.readInt32LE(
    verifierHashSizeOffset,
  );
  ...
  return {
    salt,
    encryptionVerifier,
    encryptionVerifierHash,
    encryptionVerifierHashSize,
  };
}
