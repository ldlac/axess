...
export function parseEncryptionDescriptor(
  buffer: Buffer,
  ...
  const keyEncryptor =
    parsedXML.encryption.keyEncryptors.keyEncryptor["p:encryptedKey"];
  ...
  return {
    ...
    passwordKeyEncryptor: {
      ...
      encrypted: {
        ...
        verifierHashInput: Buffer.from(
          keyEncryptor.encryptedVerifierHashInput,
          "base64",
        ),
        verifierHashValue: Buffer.from(
