...
const ENC_VERIFIER_INPUT_BLOCK = [
  0xfe, 0xa7, 0xd2, 0x76, 0x3b, 0x4b, 0x9e, 0x79,
];
const ENC_VERIFIER_VALUE_BLOCK = [
  0xd7, 0xaa, 0x0f, 0x6d, 0x30, 0x61, 0x34, 0x4e,
];
const ENC_VALUE_BLOCK = [0x14, 0x6e, 0x0b, 0xe7, 0xab, 0xac, 0xd0, 0xd6];

export function createAgileCodecHandler(
  encodingKey: Buffer,
  encryptionProvider: Buffer,
  password: Buffer,
): CodecHandler {
  const { keyData, passwordKeyEncryptor } =
    parseEncryptionDescriptor(encryptionProvider);
  ...
  const decryptPage: DecryptPage = (b, pageNumber) => {
    ...
    const iv = hash(
      keyData.hash.algorithm,
      [keyData.salt, pageEncodingKey],
      keyData.blockSize,
    );
  ...
  const verifyPassword: VerifyPassword = () => {
    ...
    const verifierHash = decryptVerifierHashValue(
      password,
    ...
    if (testHash.length % blockSize != 0) {
      const hashLength =
        Math.floor((testHash.length + blockSize - 1) / blockSize) * blockSize;
...
function decryptVerifierHashInput(
  password: Buffer,
  passwordKeyEncryptor: PasswordKeyEncryptor,
): Buffer {
  const key = deriveKey(
...
function decryptVerifierHashValue(
  password: Buffer,
  passwordKeyEncryptor: PasswordKeyEncryptor,
): Buffer {
  const key = deriveKey(
