...
export function createOfficeCodecHandler(
  databaseDefinitionPage: Buffer,
  password: string,
): CodecHandler {
  const encodingKey = databaseDefinitionPage.slice(
    KEY_OFFSET,
    KEY_OFFSET + KEY_SIZE,
  );
  ...
  const infoLength = databaseDefinitionPage.readUInt16LE(
  ...
  switch (version) {
    case "4.4":
      ...
      return createAgileCodecHandler(
        encodingKey,
