...
function getMsysComplexColumnsPage(database: Database): number {
  ...
  const complexColRow = msysObjectsData.find(
    (r) => r.Name === MSYS_COMPLEX_COLUMNS_TABLE,
  );
  if (!complexColRow) {
...
export function getComplexColumnsData(
  database: Database,
): ComplexColumnsData[] {
  const msysComplexColumnsPage = getMsysComplexColumnsPage(database);
  const msysComplexColumns = new Table(
    MSYS_COMPLEX_COLUMNS_TABLE,
    database,
    msysComplexColumnsPage,
  );
