...
export function readComplex(
  ...
  try {
    ...
    const { tableName: flatTableName, firstPage: flatTableFirstPage } =
      resolveFlatTableForComplexColumn(database, complexColumnDefinition);
    ...
    const foreignKeyColumn = flatTable
      ...
      .find(
        (c) =>
          ...
          !c.autoLong &&
          !ATTACHMENT_TYPE_COLUMN_NAMES.has(c.name),
      );
    if (!foreignKeyColumn) {
    ...
    const matchingRows = flatData.filter(
      (row) => row[foreignKeyColumn.name] === foreignKey,
    );
