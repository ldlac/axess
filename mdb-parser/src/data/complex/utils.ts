...
export function resolveFlatTableForComplexColumn(
  ...
  for (const row of complexColsData) {
    ...
    const tableMatch =
      typeof rowConceptualTableId === "number" &&
      rowConceptualTableId === tableDefPageMasked;
    if (!tableMatch) {
      continue;
    }

    const complexTypeIdMatch =
      typeof row.ComplexTypeObjectID === "number" &&
      row.ComplexTypeObjectID === column.complex.typeId;
    const complexIdMatch =
      typeof row.ComplexID === "number" &&
      row.ComplexID === column.complex.typeId;
    const columnNameMatch =
      typeof row.ColumnName === "string" &&
      row.ColumnName.toLowerCase() === column.name.toLowerCase();
    ...
    const flatTableRow = msysObjectsData.find(
      (r) => maskTableId(r.Id) === flatTableId,
    );
    if (!flatTableRow) {
