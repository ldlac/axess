...
export class Database {
  ...
  constructor(buffer: Buffer, password: string) {
    ...
    this.#codecHandler = createCodecHandler(
      this.#databaseDefinitionPage,
      password,
    );
  ...
  #getPasswordMask(): Buffer | null {
    ...
    const dateValue = this.#databaseDefinitionPage.readDoubleLE(
  ...
  getPage(page: number): Buffer {
    ...
    const pageBuffer = this.#buffer.slice(
      offset,
      offset + this.#format.pageSize,
    );
...
function decryptHeader(buffer: Buffer, format: JetFormat): void {
  const decryptedBuffer = decryptRC4(
    ...
    buffer.slice(
      ENCRYPTION_START,
      ENCRYPTION_START + format.databaseDefinitionPage.encryptedSize,
    ),
  );
