...
export function parseEncryptionHeader(
  ...
  const cryptoAlgorithm = getCryptoAlgorithm(
    buffer.readInt32LE(CRYPTO_OFFSET),
    flags,
  );
  const hashAlgorithm = getHashAlgorithm(
    buffer.readInt32LE(HASH_OFFSET),
    flags,
  );
  const keySize = getKeySize(
    buffer.readInt32LE(KEY_SIZE_OFFSET),
    cryptoAlgorithm,
    getCSPName(buffer.slice(32)),
  );
  ...
  if (
    !isInRange(cryptoAlgorithm.keySizeMin, cryptoAlgorithm.keySizeMax, keySize)
  ) {
...
function getCryptoAlgorithm(id: number, flags: number): CryptoAlgorithm {
  ...
  const algorithm = Object.values(CRYPTO_ALGORITHMS).find(
    (alg) => alg.id === id,
  );
  if (algorithm) {
    return algorithm;
