...
export function createRC4CryptoAPICodecHandler(
  ...
  const encryptionHeader = parseEncryptionHeader(
    headerBuffer,
    VALID_CRYPTO_ALGORITHMS,
    VALID_HASH_ALGORITHMS,
  );
  const encryptionVerifier = parseEncryptionVerifier(
    encryptionProvider,
  ...
  return {
    ...
    verifyPassword: () => {
      ...
      const testHash = fixBufferLength(
        hash("sha1", [verifier]),
        encryptionVerifier.encryptionVerifierHashSize,
      );
